...
/// <summary>
/// Утилиты для сравнения и проекции таблиц <see cref="DataTable"/>.
/// </summary>
public static class DataTableComparer
{
    /// <summary>
    /// Создает новую таблицу с выбранными колонками из исходного набора.
    /// </summary>
    /// <param name="source">Исходная таблица.</param>
    /// <param name="requestedColumns">Список колонок, которые нужно взять из <paramref name="source"/>.</param>
    /// <returns>Новая таблица с выбранными колонками и всеми строками исходной таблицы.</returns>
    public static DataTable SelectColumns(DataTable source, params string[] requestedColumns)
    {
        ...
        if (requestedColumns == null || requestedColumns.Length == 0)
            throw new ArgumentException("Необходимо указать хотя бы одну колонку.", nameof(requestedColumns));
        ...
        for (var i = 0; i < requestedColumns.Length; i++)
        {
            if (string.IsNullOrWhiteSpace(requestedColumns[i]))
                throw new ArgumentException("Имя колонки не может быть пустым.", nameof(requestedColumns));
    ...
    private static string GetRequiredColumnName(DataTable table, string requestedName, bool ignoreCase = true)
    {
        if (table == null)
        ...
        throw new InvalidOperationException(
            string.Format("Колонка '{0}' не найдена в результирующем наборе.", requestedName));
    }

    /// <summary>
    /// Проверяет равенство двух таблиц и выбрасывает исключение при различии.
    /// </summary>
    /// <param name="expected">Ожидаемая таблица.</param>
    /// <param name="actual">Фактическая таблица.</param>
    ...
    /// <summary>
    /// Форматирует таблицу в текстовый вид для сообщений об ошибках и логов.
    /// </summary>
    /// <param name="table">Таблица для форматирования.</param>
    /// <param name="maxRows">Максимум выводимых строк.</param>
    /// <param name="maxCellLength">Максимальная длина значения ячейки.</param>
    /// <returns>Текстовая таблица.</returns>
    ...
    /// <summary>
    /// Сравнивает две таблицы и возвращает подробный результат сравнения.
    /// </summary>
    /// <param name="expected">Ожидаемая таблица.</param>
    /// <param name="actual">Фактическая таблица.</param>
    /// <param name="options">Опции сравнения.</param>
    /// <returns>Результат сравнения с признаком равенства и diff-информацией.</returns>
    public static DataTableComparisonResult Compare(
        DataTable expected,
        DataTable actual,
        DataTableComparisonOptions options = null)
    {
        options ??= new DataTableComparisonOptions();
        ...
        if (expected == null)
        {
            ...
            {
                IsEqual = false,
                DiffMessage = "Ожидаемая таблица DataTable равна null.",
                DiffTable = null
            };
        }
        ...
        if (actual == null)
        {
            ...
            {
                IsEqual = false,
                DiffMessage = "Фактическая таблица DataTable равна null.",
                DiffTable = null
            };
        }
        ...
        if (expected.Columns.Count != actual.Columns.Count)
        {
            return new DataTableComparisonResult
            {
                ...
                DiffMessage = string.Format(
                    "Не совпадает количество колонок. Ожидалось: {0}, фактически: {1}.",
        ...
        if (options.IgnoreColumnOrder)
        {
            foreach (DataColumn expectedColumn in expected.Columns)
            {
                ...
                if (!exists)
                {
                    ...
                    {
                        IsEqual = false,
                        DiffMessage = string.Format("Колонка '{0}' не найдена в фактической таблице.", expectedColumn.ColumnName),
                        DiffTable = null
                    };
                }
            ...
            for (var i = 0; i < expected.Columns.Count; i++)
            {
                ...
                if (!actualName.Equals(expectedName, columnNameComparison))
                {
                    return new DataTableComparisonResult
                    {
                        IsEqual = false,
                        DiffMessage = string.Format(
                            "Несовпадение колонки по индексу {0}. Ожидалось: '{1}', фактически: '{2}'.",
                            i,
                            expectedName,
                            actualName),
                        DiffTable = null
        ...
        var expectedRows = BuildRows(expected, logicalColumns, options);
        var actualRows = BuildRows(actual, logicalColumns, options);
        ...
        if (orderSensitive && sameMultiset)
        {
            ...
            message.AppendLine("Таблицы DataTable не совпадают: отличается порядок строк.");
            message.AppendLine("Ожидаемый порядок:");
            message.AppendLine(FormatAsTextTable(expected, options.MaxDiffRows, options.MaxCellLength));
            message.AppendLine("Фактический порядок:");
        ...
        diffMessage.AppendLine("Таблицы DataTable не совпадают.");
        diffMessage.AppendLine("Легенда для колонки '_m_':");
        diffMessage.AppendLine("< - строка есть только в ожидаемой таблице");
        diffMessage.AppendLine("> - строка есть только в фактической таблице");
        diffMessage.AppendLine("= - строка есть в обеих таблицах");
    ...
    private static Dictionary<string, int> BuildSignatureCounts(List<NormalizedRow> rows)
    {
        ...
        {
            counts.TryGetValue(row.Signature, out int current);
        ...
        DataTableComparisonOptions options)
    {
        ...
        foreach (var key in allKeys)
        {
            ...
            if (!expectedBuckets.TryGetValue(key, out Queue<NormalizedRow> expectedQueue))
                expectedQueue = new Queue<NormalizedRow>();
            if (!actualBuckets.TryGetValue(key, out Queue<NormalizedRow> actualQueue))
    ...
    private static Dictionary<string, Queue<NormalizedRow>> BuildBuckets(List<NormalizedRow> rows)
    {
        ...
        foreach (var row in rows)
        {
            if (!buckets.TryGetValue(row.Signature, out Queue<NormalizedRow> queue))
            {
